...
export class ParameterBlock {
  ...
  async setBuffer(value: number) {
    await this.page.getByTestId('Buffer selection form').fill(value.toString());
  }
  async setWKT(value: string = 'POLYGON ((-64.8 32.3, -65.5 18.3, -80.3 25.2, -64.8 32.3))') {
  ...
  async setNbTaxonPerLine(value: number) {
    await this.page.getByTestId('Number of taxon per line form').fill(value.toString());
  }
  async toggleShowFilters(value = null) {
    const checkBox = await this.page.getByTestId('Show/Hide results filters checkbox');
    value ? await checkBox.check() : await checkBox.uncheck();
  }
  async toggleMapEditable(value: boolean = null) {
    const checkbox = await this.page.getByTestId('Show/Hide drawing tools checkbox');
    value ? await checkbox.check() : await checkbox.uncheck();
  }
  async setMode(modeOptionString: string) {
    const modeSelect = await this.page.getByTestId('Widget type selection form');
  ...
  async toggleHybridTaxonList(value) {
    const checkBox = await this.page.getByTestId('Show/Hide taxon list mode switch');
    value ? await checkBox.check() : await checkBox.uncheck();
  }
  async setXY(x, y) {
    const builder = new UrlBuilder(this.page);
  ...
  async changeSource(connectorSelected: SOURCES, params: [GeoNatureParams, GBIFParams]) {
    const builder = new UrlBuilder(this.page);
    builder.setParameter({ ...params, [Parameter.connector]: connectorSelected });
  }
}
